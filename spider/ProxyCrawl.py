...
class ProxyCrawl(object):
    ...
    def run(self):
        while True:
            ...
            str = 'usable ip:%d' % len(self.proxies)+'\r\n'
            if len(self.proxies) < MINNUM:
                str += 'now ip num < MINNUM,start crawling...'
                sys.stdout.write(str + "\r\n")
                sys.stdout.flush()
                spawns = []
                for p in parserList:
                ...
                str += 'now ip num meet the requirement,wait~~~~~'
                sys.stdout.write(str + "\r\n")
                sys.stdout.flush()
...
if __name__ == "__main__":
    ...
    p2.start()
    p3.start()
    # spider = ProxyCrawl(q1, DB_PROXY_NUM,'http://112.87.69.236:9999')
    # spider.run()